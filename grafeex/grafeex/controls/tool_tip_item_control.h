...
#ifndef GRAFEEX_TOOL_TIP_ITEM_CONTROL_H
#define GRAFEEX_TOOL_TIP_ITEM_CONTROL_H
...
namespace grafeex{
	namespace window{
		namespace controls{
			...
			class tool_tip_item : public gui::generic_object{
			public:
				...
				typedef tool_tip_item_style::type style_type;
				...
				tool_tip_item(gui_object_type &parent, gui_object_type &owner, const std::wstring &value, const rect_type &bounding_rect = rect_type());

				tool_tip_item(const sibling_type &sibling, gui_object_type &owner, const std::wstring &value, const rect_type &bounding_rect = rect_type());
				...
				virtual tool_tip_item &text(const std::wstring &value);

				virtual const std::wstring &text() const;

				virtual tool_tip_item &bounding_rect(const rect_type &value);

				virtual const rect_type &bounding_rect() const;

				virtual tool_tip_item &title(const std::wstring &value);

				virtual const std::wstring &title() const;

				virtual tool_tip_item &max_width(int value);

				virtual int max_width() const;

				virtual tool_tip_item &font(const wchar_t *family, font_weigth_type weight = font_weigth_type::DWRITE_FONT_WEIGHT_NORMAL, long size = -12);

				virtual tool_tip_item &font(font_type value);

				virtual font_type font() const;

				virtual tool_tip_item &image(gdi_obj_type value);

				virtual gdi_obj_type image() const;

			protected:
				friend class tool_tip_item_style;
				...
				virtual bool in_place_(tool_tip &parent) const;

				virtual int max_width_(tool_tip &parent) const;

				virtual const std::wstring &title_(tool_tip &parent) const;

				virtual gdi_obj_type image_(tool_tip &parent) const;

				virtual font_type font_(tool_tip &parent) const;
...
#endif /* !GRAFEEX_TOOL_TIP_ITEM_CONTROL_H */
