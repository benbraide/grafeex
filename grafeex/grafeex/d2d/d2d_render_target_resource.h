...
namespace grafeex{
	namespace d2d{
		...
		public:
			...
			typedef common::com_ptr<::ID2D1SolidColorBrush> solid_brush_type;
			typedef common::com_ptr<::ID2D1LinearGradientBrush> linear_gradient_brush_type;
			typedef common::com_ptr<::ID2D1RadialGradientBrush> radial_gradient_brush_type;
			...
			solid_brush_type::interface_type &solid_brush(const color_type &color){
				if (!solid_brush_.valid())
					target_interface_->CreateSolidColorBrush(color, solid_brush_);
				else//Update color
			...
			solid_brush_type::interface_type &solid_brush(const gdi_color_type &color){
				return solid_brush(::D2D1::ColorF(color.relative_red(), color.relative_green(), color.relative_blue(), color.relative_alpha()));
			}

			solid_brush_type::interface_type &solid_brush(){
				if (!solid_brush_.valid())
					target_interface_->CreateSolidColorBrush(::D2D1::ColorF(0.0f, 0.0f, 0.0f), solid_brush_);
				return solid_brush_;
			}

			linear_gradient_brush_type::interface_type &linear_gradient_brush(){
				return linear_gradient_brush_;
			}

			radial_gradient_brush_type::interface_type &radial_gradient_brush(){
				return radial_brush_brush_;
			}
